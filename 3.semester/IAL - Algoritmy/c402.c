...
**/
	...
	if (r_ptr != NULL){
			...
			ptr = r_ptr;
			if (r_ptr->Cont > Content){
				r_ptr = r_ptr->LPtr;
			}
	      		
	    	else if(r_ptr->Cont < Content){
				r_ptr = r_ptr->RPtr;
			}
	    	else if(r_ptr->Cont == Content){
				found = TRUE;  
...
**/
	...
	while(!SEmptyP(&stack)){
                to_delete = STopPopP(&stack); 
            
			
		if (to_delete->RPtr != NULL){
			SPushP(&stack, to_delete->RPtr);  
		}

		if (to_delete->LPtr != NULL){
			SPushP(&stack, to_delete->LPtr);  
		}           
